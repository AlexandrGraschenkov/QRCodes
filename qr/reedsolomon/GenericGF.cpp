...
Ref<GenericGFPoly> GenericGF::buildMonomial(int degree, int coefficient) {
    ...
    if (degree < 0) {
        throw ReedSolomonException("Degree must be non-negative");
    }
    if (coefficient == 0) {
        return zero;
    }
...
int GenericGF::log(int a) {
    checkInit();
    if (a == 0) {
        throw ReedSolomonException("cannot give log(0)");
    }
    return logTable[a];
}
...
int GenericGF::inverse(int a) {
    checkInit();
    if (a == 0) {
        throw ReedSolomonException("Cannot calculate the inverse of 0");
    }
