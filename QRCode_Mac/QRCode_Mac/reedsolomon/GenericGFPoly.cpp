...
Ref<GenericGFPoly> GenericGFPoly::addOrSubtract(Ref<GenericGFPoly> other) {
    assert(field_.object_ == other->field_.object_); // GenericGFPolys do not have same GenericGF field
  if (isZero()) {
    return other;
  }
  if (other->isZero()) {
...
Ref<GenericGFPoly> GenericGFPoly::multiply(Ref<GenericGFPoly> other) {
    assert(field_.object_ == other->field_.object_);// GenericGFPolys do not have same GenericGF field
...
Ref<GenericGFPoly> GenericGFPoly::multiplyByMonomial(int degree, int coefficient) {
    assert(degree >= 0);// degree must not be less then 0
  if (coefficient == 0) {
    return field_->getZero();
  }
  int size = coefficients_->size();
